...
@onready var animated_sprite = $tollethedeadsprite
...
func _ready() -> void:
	if animated_sprite:
		animated_sprite.play("toll the dead")
...
func _on_damagearea_entered(area: Area2D) -> void:
	...
	if enemy.has_method("take_dmg"):
		print("toll the dead hit enemy for ", damage, " damage!")
		enemy.take_dmg(damage)
		_on_collision() 

#handle collision
func _on_collision():
	print("toll the dead hit something!")
	queue_free()

#destroy toll the dead
